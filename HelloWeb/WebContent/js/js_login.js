$(function() {
	$("#loginbtnid").click(function() {
		//用户名不能为空
		if($("#unid").val().trim() == "" || $("#unid").val().trim() == $("#unid").attr("title")){
			$("#msg").html("用户名不能为空");
			return false;
		}
		//密码不能为空
		if($("#pwtxid").val().trim() == "" || $("#pwtxid").val().trim() == $("#pwtxid").attr("title")){
			$("#msg").html("密码不能为空");
			return false;
		}
...
function getUserAllInfosuccess(userdata) {
	console.log("userdata.status:" + userdata.status + "\nuserdata.message:" + userdata.message);
	if (userdata.status == "0000") {
		// 使用JSON.stringify将json对象转成json字符串
		// 使用setItem将json字符串暂存入sessionStorage，名称为userdatatemp
		// 接收页使用getItem取出
...
/*	var parms = {
	...
	$.ajax({
		...
		*/
		...
		//data : parms,
		dataType : "json",
		success : getProjectAllInfosuccess,
		error : function(XMLHttpRequest, textStatus, errorThrown) {
			alert("XMLHttpRequest.status:" + XMLHttpRequest.status + "\nXMLHttpRequest.readyState:" + XMLHttpRequest.readyState + "\ntextStatus:" + textStatus);
			console.log("XMLHttpRequest.status:" + XMLHttpRequest.status + "\nXMLHttpRequest.readyState:" + XMLHttpRequest.readyState + "\ntextStatus:" + textStatus);
		}
...
function getProjectAllInfosuccess(projectdata) {
	console.log("projectdata.status:" + projectdata.status + "\nprojectdata.message:" + projectdata.message);
	if (projectdata.status == "0000" || projectdata.status == "0001") {
		// 使用JSON.stringify将json对象转成json字符串
		// 使用setItem将json字符串暂存入sessionStorage，名称为projectdatatemp
		// 接收页使用getItem取出
		sessionStorage.setItem("projectdatatemp", JSON.stringify(projectdata));
		window.location.href = "jsp/project/testallprojects.jsp";
	} else {
		$("#msg").html("未知错误,请联系管理员");
	}
}
 ...
 * function(XMLHttpRequest){ this; //调用本次ajax请求时传递的options参数 } 9.complete：
